...
import { ExceptionHandlerUser } from 'src/modules/exception-handler/interfaces/exception-handler-user.interface';
import { ExceptionHandlerWorkspace } from 'src/modules/exception-handler/interfaces/exception-handler-workspace.interface';
...
import {
  ...
  TimeoutError,
  ValidationError,
} from 'src/modules/graphql/utils/graphql-errors.util';
...
export const shouldCaptureException = (
  ...
  if (
    ...
    ((exception?.extensions?.http as any)?.status ?? 500) < 500
  ) {
    return false;
  }
