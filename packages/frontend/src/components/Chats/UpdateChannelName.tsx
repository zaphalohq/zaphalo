...
const UpdateChannelName = () => {
  ...
  return (
    <>
      { isUpdateChannelName ? <div className="absolute inset-0 h-100% bg-stone-900/30 " >
        <CloseButton onClick={() => setIsUpdateChannelName(false) } 
                    right="right-120" top="top-60" />
        <form onSubmit={HandleUpdateChannelName} className="absolute top-[35%] right-[35%] w-[30rem] bg-stone-100 p-10 rounded flex flex-col gap-4">
